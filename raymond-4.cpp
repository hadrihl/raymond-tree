...
// Function to print ASCII art of the tree with ANSI colors and token holder symbol
void printTreeASCII(const node* roonodeT, const vector<int>& path, int level = 0, const string& prefix = "", bool isLast = true) {
    ...
    bool inPath = find(path.begin(), path.end(), roonodeT->id) != path.end();
    bool isCurrent = (!path.empty() && roonodeT->id == path.back());
    bool isTokenHolder = (roonodeT->id == roonodeT->holderval);

    // ANSI color codes: yellow for token holder, red for path, green for current, cyan for connectors
    string nodeColor = isTokenHolder ? "\033[33m" : (isCurrent ? "\033[32m" : (inPath ? "\033[31m" : ""));
    string resetColor = (isTokenHolder || isCurrent || inPath) ? "\033[0m" : "";
    string connectorColor = "\033[36m";
    string connector = isLast ? "└── " : "├── ";
    string tokenSymbol = isTokenHolder ? "★" : "";

    // Print current node with holderval
    cout << connectorColor << prefix << connector << resetColor 
         << nodeColor << tokenSymbol << roonodeT->id << " (h:" << roonodeT->holderval << ")" << resetColor << endl;

    // Prepare prefix for children
    string newPrefix = prefix + (isLast ? "    " : connectorColor + "│   " + resetColor);
