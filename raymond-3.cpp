...
// Function to print ASCII art of the tree with ANSI colors
void printTreeASCII(const node* roonodeT, const vector<int>& path, int level = 0, const string& prefix = "", bool isLast = true) {
    ...
    // Determine if node is in path or is the current (last) node
    bool inPath = find(path.begin(), path.end(), roonodeT->id) != path.end();
    bool isCurrent = (!path.empty() && roonodeT->id == path.back());

    // ANSI color codes: red for path nodes, green for current node, cyan for connectors
    string nodeColor = isCurrent ? "\033[32m" : (inPath ? "\033[31m" : "");
    string resetColor = inPath || isCurrent ? "\033[0m" : "";
    string connectorColor = "\033[36m";
    string connector = isLast ? "└── " : "├── ";

    // Print current node
    cout << connectorColor << prefix << connector << resetColor << nodeColor << roonodeT->id << resetColor << endl;
